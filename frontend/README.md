...
API-контракт хранится в `../contracts/openapi`. При будущей миграции на Angular новый frontend должен использовать тот же OpenAPI-контракт и не зависеть от внутренних моделей backend.
...
При переписывании на Angular рекомендуется временно держать новую реализацию рядом, например `frontend-angular/`, пока она не заменит текущий каталог `frontend/`.
