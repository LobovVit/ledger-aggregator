...
├── contracts/               # Межмодульные контракты, сейчас OpenAPI
...
OpenAPI как контракт между backend и frontend хранится в `contracts/openapi`. Источник генерации - `backend/docs`; после изменения API нужно регенерировать Swagger и обновить копию в `contracts/openapi`.
...
В `.github/workflows` разделены проверки:

- `backend.yml` - `go test ./...` и `go vet ./...` для backend.
- `frontend.yml` - `npm ci` и `npm run build` для frontend.
- `integration.yml` - сборка и smoke-запуск `docker compose`.
